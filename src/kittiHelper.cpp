/**
 * @defgroup   KITTIHELPER kitti Helper
 *
 * @brief      This file implements kitti Helper.
 *
 * @date       2020-07-14
 */
#include <iostream>
...
std::vector<float> read_lidar_data(const std::string lidar_data_path)
{
    ...
    // 这个函数的原型istream& read (char* s, streamsize n);
...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle n("~");
    // 从参数服务器中读取参数，获得数据集的路径、sequence标号
    ...
    ros::Publisher pubOdomGT = n.advertise<nav_msgs::Odometry> ("/odometry_groundtruth_in_lidar", 5);
    ...
    while (std::getline(timestamp_file, line) && ros::ok())
    {
        ...
        std::stringstream left_image_path, right_image_path;                                 // 自动填充'0'使得'0'+line_num凑成6个字符
        ...
        std::string s;
        Eigen::Matrix<double, 3, 4> gt_pose;  // 在left camera坐标系下的ground truth
        ...
        // R_i^lidar = R_cam^lidar * R_i^cam 
        Eigen::Quaterniond q = q_transform * q_w_i;
        q.normalize();
        // 这里的t恒定相差了一个t_cam^lidar
        ...
        std::cout << "totally " << lidar_data.size() / 4.0 << " points in " << line_num <<" lidar frame \n";

        std::vector<Eigen::Vector3d> lidar_points;       // 点的x,y,z坐标
        std::vector<float> lidar_intensities;            // 反射光强
        pcl::PointCloud<pcl::PointXYZI> laser_cloud;     // 一帧雷达点云
